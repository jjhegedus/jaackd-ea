<MDG.Selections model="jaackd-tools --- DBType=11;Connect=Provider=SSDB;SRC=C:\p\gh\jjhegedus\jaackd-ea\jaackd-tools.qea;UID=;PWD=;">
	<Technology id="jaackd" name="jaackd" version="0.0.15" notes="jaackd MDG for software and system architecture" filename="C:\p\gh\jjhegedus\jaackd-ea\MDG\jaackd-mdg.xml" infoURI="jaackd.com" supportURI="admin@jaackd.com"/>
	<Profiles directory="C:\p\gh\jjhegedus\jaackd-ea\MDG" files="jaackd-profile.xml"/>
	<Patterns directory="C:\p\gh\jjhegedus\jaackd-ea\MDG" files="jaackd-model-type-pattern.xml"/>
	<TagVals tags="architectureChange,Boolean,businessArchitecture,Classifier,codeModels,Color,Connector,Date,DiagramRef,Directory,enterpriseArchitecture,File,Float,GitHubRepository,ImageRef,infrastructureModel,Integer,JaackdEAAddinBroadcast,ListOfClassifier,ListOfProperties,Memo,modelTooling,Operation,overview,project,projects,Property,reference,sandbox,String,systemArchitecture,technicalArchitecture,Time,Timestamp,URL"/>
	<CodeMods>
		<CodeMod product="jaackd-csharp" codetypes="1" codetemplates="1" grammar="C:\p\gh\jjhegedus\jaackd-ea\EA Language Grammars\jaackd-csharp.nBNF" codeoptions=""/>
	</CodeMods>
	<DiagramProfile directory="C:\p\gh\jjhegedus\jaackd-ea\MDG" files="jaackd-profile-diagram.xml"/>
	<UIToolboxes directory="C:\p\gh\jjhegedus\jaackd-ea\MDG" files="jaackd-profile-toolbox.xml"/>
	<ModelTemplates>
		<Model name="jaackd-model-type-legend" description="legend which color codes elements by the model type" location="jaackd-model-type-legend" default="yes" icon="34" filter="Common"/>
		<Model name="jaackd-problem-model-no-requirements" description="template structure for a problem model with only requirements" location="jaackd-problem-model-no-requirements" default="yes" icon="34" filter="Common"/>
	</ModelTemplates>
</MDG.Selections>